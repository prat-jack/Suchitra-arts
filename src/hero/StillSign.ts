...
interface StillCfg {
  ...
  cam: { xOff: number; y: number; ty: number; fov: number; fit: number }
  bloom: number
  centerY: number
}
...
export const STILLS: Record<string, StillCfg> = {
  kanaka: {
    ...
    rough: 0.32,
    faceGlow: 0.1,
    cam: { xOff: -1.1, y: 1.9, ty: 2.05, fov: 36, fit: 1.18 },
    bloom: 0.32,
    centerY: 2.1,
  },
  marigold: {
    ...
    rough: 0.5,
    faceGlow: 0.06,
    cam: { xOff: 0.7, y: 1.8, ty: 2.1, fov: 38, fit: 1.22 },
    bloom: 0.3,
    centerY: 2.1,
  },
  basava: {
    ...
    rough: 0.28,
    faceGlow: 0.14,
    cam: { xOff: 0, y: 2.0, ty: 2.1, fov: 34, fit: 1.16 },
    bloom: 0.36,
    centerY: 2.1,
  },
  bakehouse: {
    ...
    rough: 0.55,
    faceGlow: 0.05,
    cam: { xOff: -0.8, y: 2.15, ty: 2.05, fov: 38, fit: 1.2 },
    bloom: 0.28,
    centerY: 2.05,
  },
  veda: {
    ...
    rough: 0.45,
    faceGlow: 0.07,
    cam: { xOff: 0.9, y: 1.9, ty: 2.15, fov: 36, fit: 1.25 },
    bloom: 0.3,
    centerY: 2.15,
  },
  cubbon: {
    ...
    rough: 0.4,
    faceGlow: 0.12,
    cam: { xOff: 0.6, y: 2.3, ty: 2.05, fov: 37, fit: 1.14 },
    bloom: 0.33,
    centerY: 2.05,
  },
}

function wallTexture(kind: WallKind): { map: THREE.CanvasTexture; bump: THREE.CanvasTexture } {
  const c = document.createElement('canvas')
  c.width = c.height = 512
  const ctx = c.getContext('2d')!

  const noise = (alpha: number, count: number, rMin: number, rMax: number, vMin: number, vMax: number) => {
    for (let i = 0; i < count; i++) {
      const v = vMin + Math.floor(Math.random() * (vMax - vMin))
      ctx.fillStyle = `rgba(${v},${v * 0.93},${v * 0.82},${alpha})`
      ctx.beginPath()
      ctx.arc(Math.random() * 512, Math.random() * 512, rMin + Math.random() * (rMax - rMin), 0, Math.PI * 2)
      ctx.fill()
    }
  }
  ...
  if (kind === 'stone') {
    ...
    noise(0.16, 260, 10, 46, 16, 52)
    noise(0.2, 700, 1, 5, 8, 60)
  } else if (kind === 'charcoal') {
    ctx.fillStyle = '#151210'
    ctx.fillRect(0, 0, 512, 512)
    noise(0.1, 320, 6, 26, 14, 40)
  } else if (kind === 'plaster') {
    ctx.fillStyle = '#2a231b'
    ctx.fillRect(0, 0, 512, 512)
    noise(0.09, 420, 4, 22, 26, 66)
    noise(0.12, 900, 1, 3, 20, 70)
  } else {
    ...
    for (let row = 0; row < 512 / bh; row++) {
      ...
      for (let x = -1; x < 512 / bw + 1; x++) {
        ...
        ctx.fillRect(x * bw + off + 2, row * bh + 2, bw - 4, bh - 4)
      }
    }
    noise(0.14, 700, 1, 4, 10, 50)
  }
...
export function mountStill(canvas: HTMLCanvasElement, id: string): () => void {
  ...
  const wall = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ map, bumpMap: bump, bumpScale: 2.6, roughness: 0.92 }),
  )
  ...
  scene.add(new THREE.AmbientLight(0xffe0bb, 0.26))
  const graze = new THREE.SpotLight(0xffdcae, 16, 0, 1.05, 0.9, 1.5)
  ...
  new TTFLoader().loadAsync(`${import.meta.env.BASE_URL}${cfg.font}`).then((ttf) => {
    ...
    // The halo: one LED-strip RectAreaLight behind each row, washing the wall
    // continuously — how reverse-channel letters are actually lit. Point
    // lights read as bead hotspots; an area light reads as a strip.
    RectAreaLightUniformsLib.init()
    for (const [ri, row] of rows.entries()) {
      const span = row.maxX - row.minX
      const rowH = cfg.rows[ri].size
      const strip = new THREE.RectAreaLight(cfg.halo, cfg.haloI ?? 2.2, span + rowH * 0.25, rowH * 1.05)
    ...
    const fill = new THREE.PointLight(0xffe4c0, 0.9, 14, 1.8)
    fill.position.set(cfg.cam.xOff, cfg.cam.y + 0.6, 4.2)
    scene.add(fill)

    // Downlight from above the sign — the single warm spot most premium
    // installs use; lifts letter faces and scallops the wall
    const down = new THREE.SpotLight(0xffd9a6, 5, 0, 0.55, 1.0, 1.7)
    down.position.set((minXAll(rows) + maxXAll(rows)) / 2, cfg.centerY + 3.4, 1.6)
